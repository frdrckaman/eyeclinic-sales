...
<option value="<?=$payment['id']?>"><?='Batch Name:'.$batch['name'].' | Lens:'.$lensType['name'].' | Lens Categ: '.$sale['lens_cat'].' | Lens Power:'.$sale['lens_power']. ' | Quantity:'.$sale['quantity']
    .' | Invoice:'.$sale['invoice'].' | Delivery Note:'.$sale['delivery_note'].' | Amount to be Paid:'.number_format($pay)?></option>
<?php }}elseif ($_GET['cnt'] == 'payAc'){?>
    <option value="">Select</option>
    <?php $payments=$override->getNews('payment_accessories','status',0,'customer_id',$_GET['getUid']);
foreach ($payments as $payment){
    $sale=$override->get('accessories_sale','id',$payment['sale_id'])[0];
    $batch=$override->get('batch','id',$sale['batch_id'])[0];
    $lensType=$override->get('accessories','id',$sale['accessory_id'])[0];
    $pay=$payment['required_amount']-$payment['pay_amount']?><?php }}?>
<option value="<?=$payment['id']?>"><?='Batch Name:'.$batch['name'].' | Lens:'.$lensType['name']. ' | Quantity:'.$sale['quantity']
    .' | Invoice:'.$sale['invoice'].' | Delivery Note:'.$sale['delivery_note'].' | Amount to be Paid:'.number_format($pay)?></option>
